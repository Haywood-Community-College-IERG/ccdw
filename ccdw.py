...
def ccdw() -> None:
    ...
    parser.add_argument(
        "--wStatusValidate",
        dest="wStatusValidate",
        action="store_true",
        default=False,
        help="Validate dates in with Status data but do not process data (default: don"
        "t validate dates)",
    )
    parser.add_argument(
        "--updateConfig",
    ...
    if not cfg_log_level in logger_types:
        ...
        logger_idx = min(
            [logger_types.index(i) for i in [cfg_log_level, cmd_log_level]]
        )
    ...
    logger.debug(
        f"Arguments: writedb = [{writedb}], diffs = [{diffs}], refresh = [{refresh}], wStatus = [{wStatus}], updateConfig=[{updateConfig}]"
    )
        ...
        for root, subdirs, files in os.walk(export_path):
            for file in files:
                with open(f"{root}/{file}", "r") as csvinput:
                    ...
                    df_status_only = set(df_status).symmetric_difference(
                        set(df_status_datetime)
                    )
                    ...
                    df.loc[
                        df[df_status_datetime[0]] == "", df_status_datetime[0]
                    ] = "1900-01-01"
                    ...
                    if len(df_status_datetime) == 2:
                        ...
                        newDataDatetime = (
                            df[df_status_datetime[0]] + "T" + df[df_status_datetime[1]]
                        )
                    else:
                    ...
                    df = df.drop_duplicates(
                        set(df.columns).symmetric_difference(set(df_status_only)),
                        keep="last",
                    )
                    ...
                    df_invalid = df[
                        pd.to_datetime(df[df_status_datetime[0]]) > invalid_date
                    ]
                    if len(df_invalid):
                    ...
                    if not wStatusValidate:
                        ...
                        try:
                            ...
                            for d in sorted(
                                df["DataDatetime"].dt.strftime("%Y-%m-%d").unique()
                            ):
                                logger.debug(f"DataDatetime = {d}")
                                table_records = exportObj.executeSQL_UPDATE(
                                    fn,
                                    df.loc[df[df_status_datetime[0]] == d]
                                    .groupby(file_keys, as_index=False)
                                    .last(),
                                    Audit_Key,
                                )
                ...
                if error_flag == "N" and not wStatusValidate:
                    ...
                    archive(
                        df, "", file, archive_path, export_path, cfg, createInitial=True
                    )
            ...
            for subdir in subdirs:
                ...
                for i in range(len(filelist)):
                    ...
                    try:
                        ...
                        inputFrame = inputFrame.where(
                            pd.notnull(inputFrame), None
                    ...
                    if "DataDatetime" in inputFrame.columns:
                        pass
                    else:
                        inputFrame["DataDatetime"] = datetime.datetime.now()

                    # Get a sorted list of a CSV file named as the file with the version added in the archive folder.
                    #     Example: For ACAD_CREDENTIALS_1001, look for ACAD_CREDENTIALS_1001_LF.csv in the archive folder.
                    ...
                    if writedb:
                        ...
                        if df.shape[0] > 0:
                            try:
                                logger.debug(
                                    f"{timestamp()} SQL_UPDATE: {file} with {df.shape[0]} rows"
                                )

                                records = exportObj.executeSQL_UPDATE(
                                    sqlName, df, Audit_Key
                                )

                                logger.debug(
                                    f"{timestamp()} SQL_UPDATE: {file} with {records} rows [DONE]"
                                )
                                ...
                                logger.error(
                                    f"'---Error in file: {file} -- the folder will be skipped"
                                )
                                error_flag = "Y"

                        else:
                            logger.debug(
                                f"{timestamp()} SQL_UPDATE: No updated data for {file}"
                            )
...
def createDiff(cf, lf):
    ...
    col_names = full_set[
        full_set.columns.difference(["DataDatetime", "version"])
    ].columns
...
@logger.catch
def archive(
    df, subdir, file, archive_path, export_path, cfg, diffs=True, createInitial=True
):
