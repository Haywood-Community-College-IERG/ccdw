...
ELSE AGE5=1;
ProgCode=GradProg;
IF (SUBSTR(GradProg,1,1)) EQ 'A' THEN AWLEVEL=3;
ELSE IF (SUBSTR(GradProg,1,1)) EQ 'D' THEN AWLEVEL=2;
ELSE IF (SUBSTR(GradProg,1,1)) EQ 'C' THEN AWLEVEL=1;
IF (SUBSTR(GradProg,1,1)) EQ 'A' THEN CTLEVEL=3;
ELSE IF (SUBSTR(GradProg,1,1)) EQ 'D' THEN CTLEVEL=2;
ELSE IF (SUBSTR(GradProg,1,1)) EQ 'C' THEN CTLEVEL=1;
...
FILENAME OUTABCD "&Pth01&Pth02&Pth03\CompletionsPartABCD_FINAL.txt";
DATA _NULL_;
FILE OUTABCD;
/* Part A */
...
ODS LISTING CLOSE;
ODS HTML CLOSE;
ODS MARKUP TAGSET=CHTML BODY="&Pth01&Pth02&Pth03\IPEDS_Completions_TotalCredentialsAndStudents.html";

DATA MissingCIP;
SET GradStudCIP01h;
PROC SORT NODUPKEY DATA=MissingCIP; BY ADCPersonID;
PROC PRINT DATA=MissingCIP;
VAR ADCPersonID GradProg;
