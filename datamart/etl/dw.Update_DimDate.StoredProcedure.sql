USE [IERG]
GO
/****** Object:  StoredProcedure [dw].[Update_DimDate]    Script Date: 1/11/2018 10:12:45 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dw].[Update_DimDate] 
...
AS BEGIN
/*
-- To REBUILD DimDate table, run this code.
EXECUTE
    dw.Update_DimDate @Start_Date = NULL, 
                       @End_Date = NULL, 
                       @Truncate_Table = 'TRUE', 
...
-- To UPDATE DimDate table, run this code.
EXECUTE
    dw.Update_DimDate Start_Date = NULL, 
                      End_Date = NULL, 
...
--* */
    ...
    ELSE BEGIN
        ...
        IF @Delete_From_Date IS NOT NULL BEGIN
            ...
            DELETE FROM dw.DimDate WHERE Full_Date >= @Delete_From_Date;
    ...
    END
    ELSE BEGIN
        SELECT @l_Start_Date = COALESCE(DATEADD(dd, 1, MAX(Full_Date)), @d_Start_Date) FROM dw.DimDate;
    ...
    INSERT dw.DimDate WITH (TABLOCKX)
    SELECT
        ...
        USING (SELECT DISTINCT
                      ...
                      YEAR(Current_Terms.[TERM.START.DATE]) * 10 
                          + CASE LTRIM(RTRIM(SUBSTRING(Current_Terms.[TERMS.ID], 5, 3)))
                                 ...
                                 WHEN 'CE2' THEN 2 
                                 WHEN 'CE3' THEN 5 
                                 ELSE NULL END AS Term_Index,
                      YEAR(Current_Terms.[TERM.START.DATE]) * 10 
                        + CASE WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 2) = 'SP' THEN - 4 
                                WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 2) = 'SU' THEN - 2 
                                WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 2) = 'FA' THEN 1 
                                WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 3) = 'CE1' THEN - 5 
                                WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 3) = 'CE2' THEN - 3 
                                WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 3) = 'CE3' THEN 0 
                                ELSE 4 END AS Term_Sort,
                      /* Academic_Year */
                      CASE WHEN Current_Terms.[TERMS.ID] IS NULL
                           ...
                           WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 2) IN ('FA', 'WI', 'SP', 'SU')
                                THEN CAST(Current_Terms.[TERM.REPORTING.YEAR] AS VARCHAR) + '-' + CAST(Current_Terms.[TERM.REPORTING.YEAR] + 1 AS VARCHAR) 
                      ...
                      CONVERT(DATE, (CASE WHEN SUBSTRING(This_Date.YYYYMMDD, 5, 4) <= '0630'
                                          ...
                                          ELSE This_Date.YYYY END) + '0701', 112) AS Academic_Year_Start_Date,
                      /* Academic_Year_Start_Date_Key */
                      RTRIM(LTRIM(CASE WHEN SUBSTRING(This_Date.YYYYMMDD, 5, 4) <= '0630' 
                                            THEN CAST(This_Date.Previous_Year AS CHAR(4))
                                       ELSE This_Date.YYYY END)) + '0701' AS Academic_Year_Start_Date_Key,
                      /* Academic_Year_End_Date */
                      CONVERT(DATE, (CASE WHEN SUBSTRING(This_Date.YYYYMMDD, 5, 4) <= '0630'
                                               THEN This_Date.YYYY
                                          ELSE CAST(This_Date.Next_Year AS CHAR(4)) END) + '0630', 112) AS Academic_Year_End_Date,
                      /* Academic_Year_End_Date_Key */
                      RTRIM(LTRIM(CASE WHEN SUBSTRING(This_Date.YYYYMMDD, 5, 4) <= '0630'
                                            THEN This_Date.YYYY
                                       ELSE CAST(This_Date.Next_Year AS CHAR(4)) END)) + '0630' AS Academic_Year_End_Date_Key,
                      /* Reporting_Year */
                      Current_Terms.[TERM.REPORTING.YEAR] AS Reporting_Year,
                      CASE WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 2) IN ('CE', 'FA', 'WI', 'SP', 'SU') 
                                THEN CAST(Current_Terms.[TERM.REPORTING.YEAR] AS VARCHAR) + '-' + CAST(Current_Terms.[TERM.REPORTING.YEAR] + 1 AS VARCHAR) 
                           ELSE NULL END AS Reporting_Academic_Year,
                      /* Semester */
                      ...
                      CASE WHEN This_Date.Full_Date <= Current_Terms.[TERM.CENSUS.DATES] 
                           ...
                           ELSE DATEDIFF(dd, This_Date.Full_Date, Next_Terms.[TERM.CENSUS.DATES]) END AS Days_to_Next_Census,
                      CASE WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 2) = 'FA'
                                AND This_Date.Full_Date <= Current_Terms.[TERM.CENSUS.DATES]
                                THEN DATEDIFF(dd, This_Date.Full_Date, Current_Terms.[TERM.CENSUS.DATES])
                           ELSE DATEDIFF(dd, This_Date.Full_Date, Fall_Terms.[TERM.CENSUS.DATES]) END AS Days_to_Fall_Census,
                      CASE WHEN SUBSTRING(Current_Terms.[TERMS.ID], 5, 2) = 'SP'
                                AND This_Date.Full_Date <= Current_Terms.[TERM.CENSUS.DATES]
                                THEN DATEDIFF(dd, This_Date.Full_Date, Current_Terms.[TERM.CENSUS.DATES])
                           ELSE DATEDIFF(dd, This_Date.Full_Date, Spring_Terms.[TERM.CENSUS.DATES]) END AS Days_to_Spring_Census,
                      /* Days after Census */
                      ...
                      YEAR(Current_Terms_CE.[TERM.START.DATE]) * 10 
                          + CASE LTRIM(RTRIM(SUBSTRING(Current_Terms_CE.[TERMS.ID], 5, 3)))
                                 ...
                                 WHEN 'CE2' THEN 2 
                                 WHEN 'CE3' THEN 5 
                                 ELSE NULL END AS CE_Term_Index,
                      YEAR(Current_Terms_CE.[TERM.START.DATE]) * 10 
                        + CASE WHEN SUBSTRING(Current_Terms_CE.[TERMS.ID], 5, 3) = 'CE1' THEN - 5 
                                WHEN SUBSTRING(Current_Terms_CE.[TERMS.ID], 5, 3) = 'CE2' THEN - 3 
                                WHEN SUBSTRING(Current_Terms_CE.[TERMS.ID], 5, 3) = 'CE3' THEN 0 
                                ELSE 4 END AS CE_Term_Sort,
                      /* CE_Academic_Year */
                      CASE WHEN SUBSTRING(Current_Terms_CE.[TERMS.ID], 5, 2) = 'CE' THEN 
                                CASE WHEN SUBSTRING(Current_Terms_CE.[TERMS.ID], 5, 3) IN ('CE', 'CE3') 
                                          THEN CAST(DATEPART(yyyy, Current_Terms_CE.[TERM.END.DATE]) AS VARCHAR) + '-' + CAST(DATEPART(yyyy, Current_Terms_CE.[TERM.END.DATE]) + 1 AS VARCHAR) 
                                     ELSE CAST(DATEPART(yyyy, Current_Terms_CE.[TERM.END.DATE]) - 1 AS VARCHAR) + '-' + CAST(DATEPART(yyyy, Current_Terms_CE.[TERM.END.DATE]) AS VARCHAR) 
                                     END 
                           ELSE NULL 
                           END AS CE_Academic_Year,
                      ...
                      /* CE_Reporting_Year */
                      Current_Terms_CE.[TERM.REPORTING.YEAR] AS CE_Reporting_Year,
                      CASE WHEN SUBSTRING(Current_Terms_CE.[TERMS.ID], 5, 2) IN ('CE') 
                                THEN CAST(Current_Terms_CE.[TERM.REPORTING.YEAR] AS VARCHAR) + '-' + CAST(Current_Terms_CE.[TERM.REPORTING.YEAR] + 1 AS VARCHAR) 
