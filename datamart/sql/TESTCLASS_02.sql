WITH cs AS (
	...
	SELECT LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN lag.[CRS.NO] ELSE '' END + REPLICATE(' ', 10), 10)
		 ...
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN CAST(lag.[COURSE.SEC.MEETING.ADDDATE] as varchar) ELSE '' END + REPLICATE(' ', 10), 10)
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN CAST(lag.[COURSE.SEC.MEETING.ADDOPR] as varchar) ELSE '' END + REPLICATE(' ', 10), 10)
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN CAST(lag.[COURSE.SEC.MEETING.CHGDATE] as varchar) ELSE '' END + REPLICATE(' ', 10), 10)
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN lag.[COURSE.SEC.MEETING.CHGOPR] ELSE '' END + REPLICATE(' ', 10), 10)
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN CAST(lag.[CSM.START.DATE] as varchar) ELSE '' END + REPLICATE(' ', 10), 10)
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN CAST(lag.[CSM.START.TIME] as varchar) ELSE '' END + REPLICATE(' ', 10), 10)
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN CAST(lag.[CSM.END.DATE] as varchar) ELSE '' END + REPLICATE(' ', 10), 10)
		 , LEFT(CASE WHEN lag.[CRS.NO] <> lag.CRS_LAG THEN CAST(lag.[CSM.END.TIME] as varchar) ELSE '' END + REPLICATE(' ', 10), 10)
